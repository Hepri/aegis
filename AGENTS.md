...
- Earn kiosk account `AegisTasks` (display «Задачки», no password): Assigned Access / Shell launches Edge kiosk to `/earn`; per-user firewall allows that account only to the Aegis server; HKCU policies disable Task Manager/Run/CMD; child-account internet unchanged.
