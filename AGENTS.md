...
- Earn kiosk account `AegisTasks` (display «Задачки», no password): Assigned Access launches `aegis-client.exe` in `earn-kiosk` mode; firewall allowlists that exe only to the Aegis server (child-account internet unchanged).
