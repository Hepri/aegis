...
- Prefers deploy via `systemctl stop` → ship → `systemctl start` when passwordless sudo works, rather than manually killing the server process.
...
- Wants earn-time: child solves server-hosted tasks on the same PC via a separate restricted Windows account, accumulates bonus minutes, then redeems with buy-time for any amount within per-day purchase windows (no overnight quiet-hour spill); sticky active question across re-login; wrong answers and “another question” can deduct configurable minutes; unused redeemed time refundable early; daily limit applies to spending, not earning; earn UI errors in Russian and should show how much was already spent.
...
- Earn-time: `/earn` UI + per-user minute wallet; multi-subject question banks with admin enable/edit (some sections repeatable); redeem grants `temporary_access`; operation audit log; admin clear-wallet, purchase toggle, max balance, daily spend limit, and per-weekday purchase windows.
