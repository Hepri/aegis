...
- In the admin activity UI, wants apps nested under sessions with total time per app only — not a focus-event timeline.
...
- `session-agent` is launched hidden (`CREATE_NO_WINDOW` / `SW_HIDE` / `FreeConsole`); after service restart, schedule lock still applies on the first poll even when prior in-memory state is empty.
